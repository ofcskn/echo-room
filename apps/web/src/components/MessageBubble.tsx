      ...
      <div 
        className={`max-w-[85%] md:max-w-[70%] px-4 py-2 text-sm md:text-base shadow-sm relative transition-all ${
          isSelf 
            ...
            : 'bg-brand-surfaceHighlight text-gray-200 border border-white/5 rounded-2xl rounded-tl-sm'
        }`}
      >
        <div className="flex flex-col min-w-[3rem]">
          <p className="leading-relaxed whitespace-pre-wrap break-words">{message.content}</p>
          <div className={`text-[10px] mt-0.5 flex items-center justify-end gap-1 select-none ${isSelf ? 'text-[#05080a]/60 font-semibold' : 'text-gray-500'}`}>
            <span>{time}</span>
            {isSelf && (
...
export const SystemMessage: React.FC<{ text: string }> = ({ text }) => (
  <div className="flex justify-center my-6 opacity-75">
    <span className="bg-brand-surface/40 backdrop-blur-sm text-gray-400 text-[10px] uppercase tracking-widest px-4 py-1 rounded-full border border-white/5">
      {text}
    </span>
  </div>
);
